...
public class AllocationUsage extends ConnectionUsage {
  ...
  protected List<ReferenceValue> allocationDefinition = new ArrayList<>();

  public AllocationUsage(@NotNull String id) {
    super(id);
  }
  ...
  public Concept getClassifier() {
    return SysmlLanguage.getInstance().getAllocationUsage();
  }

  public int addToAllocationDefinition(ReferenceValue referenceValue, int index) {
    if (index > allocationDefinition.size()) {
      throw new IllegalArgumentException("Index must be less than or equal to size");
    }
    if (partitionObserverCache != null) {
      partitionObserverCache.referenceValueAdded(this, this.getClassifier().requireReferenceByName("allocationDefinition"), index, referenceValue);
    }
    allocationDefinition.add(index, referenceValue);
    return allocationDefinition.size() - 1;
  }

  public List<ReferenceValue> getAllocationDefinition() {
    return allocationDefinition;
  }

  public int addToAllocationDefinition(AllocationDefinition referred) {
    return addToAllocationDefinition(new ReferenceValue(referred, null), allocationDefinition.size());
  }

  public int addToAllocationDefinition(AllocationDefinition referred, int index) {
    return addToAllocationDefinition(new ReferenceValue(referred, null), index);
  }

  public void clearAllocationDefinition() {
    while (!allocationDefinition.isEmpty()) {
            removeFromAllocationDefinition(0);
        };
  }

  public void removeFromAllocationDefinition(@NotNull ReferenceValue child) {
    int index = allocationDefinition.indexOf(child);
             if (index == -1) {
                 throw new IllegalArgumentException("Child not found: " + child);
             }
             removeFromAllocationDefinition(index);;
  }

  public void removeFromAllocationDefinition(int index) {
    if (allocationDefinition.size() > index) {

            ReferenceValue removed = allocationDefinition.remove(index);
            if (partitionObserverCache != null) {
              partitionObserverCache.referenceValueRemoved(this, getClassifier().requireReferenceByName("allocationDefinition"), index, removed);
            }
          } else {
            throw new IllegalArgumentException(
                "Invalid index "
                    ...
                    + allocationDefinition.size());
          }
  }

  public void setAllocationDefinition(@NotNull List<? extends ReferenceValue> newValue) {
    clearAllocationDefinition();
          for (ReferenceValue referenceValue : newValue) {
              addToAllocationDefinition(referenceValue, allocationDefinition.size());
          }
  }
  ...
  public List<ReferenceValue> getReferenceValues(@NotNull Reference reference) {
    ...
    if (Objects.equals(reference.getKey(), "sysml-AllocationUsage-allocationDefinition")) {
      return allocationDefinition;
    }
  ...
  public int addReferenceValue(Reference reference, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-AllocationUsage-allocationDefinition")) {
      return addToAllocationDefinition(referredNode, allocationDefinition.size());
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public int addReferenceValue(Reference reference, int index, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-AllocationUsage-allocationDefinition")) {
      return addToAllocationDefinition(referredNode, index);
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public void setReferenceValues(@NotNull Reference reference,
    ...
    if (Objects.equals(reference.getKey(), "sysml-AllocationUsage-allocationDefinition")) {
      setAllocationDefinition(values);
      return;
    }
  ...
  public void setReferred(@NotNull Reference reference, int index, @Nullable Node referredNode) {
    ...
    if (index < 0) throw new IllegalArgumentException("index should be non-negative");;
    if (Objects.equals(reference.getKey(), "sysml-AllocationUsage-allocationDefinition")) {
      if (index >= allocationDefinition.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = allocationDefinition.get(index);
      allocationDefinition.set(index, original.withReferred(referredNode));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-ConnectionUsage-connectionDefinition")) {
  ...
  public void setResolveInfo(@NotNull Reference reference, int index,
    ...
    if (Objects.equals(reference.getKey(), "sysml-AllocationUsage-allocationDefinition")) {
      if (index >= allocationDefinition.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = allocationDefinition.get(index);
      allocationDefinition.set(index, original.withResolveInfo(resolveInfo));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-ConnectionUsage-connectionDefinition")) {
      if (index >= connectionDefinition.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
