...
public class EnumerationDefinition extends AttributeDefinition {
  @NotNull
  private String id;
  ...
  protected List<ReferenceValue> enumeratedValue = new ArrayList<>();

  public EnumerationDefinition(@NotNull String id) {
    super(id);
  }
  ...
  public Concept getClassifier() {
    return SysmlLanguage.getInstance().getEnumerationDefinition();
  }

  public int addToEnumeratedValue(ReferenceValue referenceValue, int index) {
    if (index > enumeratedValue.size()) {
      throw new IllegalArgumentException("Index must be less than or equal to size");
    }
    if (partitionObserverCache != null) {
      partitionObserverCache.referenceValueAdded(this, this.getClassifier().requireReferenceByName("enumeratedValue"), index, referenceValue);
    }
    enumeratedValue.add(index, referenceValue);
    return enumeratedValue.size() - 1;
  }

  public List<ReferenceValue> getEnumeratedValue() {
    return enumeratedValue;
  }

  public int addToEnumeratedValue(EnumerationUsage referred) {
    return addToEnumeratedValue(new ReferenceValue(referred, null), enumeratedValue.size());
  }

  public int addToEnumeratedValue(EnumerationUsage referred, int index) {
    return addToEnumeratedValue(new ReferenceValue(referred, null), index);
  }

  public void clearEnumeratedValue() {
    while (!enumeratedValue.isEmpty()) {
            removeFromEnumeratedValue(0);
        };
  }

  public void removeFromEnumeratedValue(@NotNull ReferenceValue child) {
    int index = enumeratedValue.indexOf(child);
             if (index == -1) {
                 throw new IllegalArgumentException("Child not found: " + child);
             }
             removeFromEnumeratedValue(index);;
  }

  public void removeFromEnumeratedValue(int index) {
    if (enumeratedValue.size() > index) {

            ReferenceValue removed = enumeratedValue.remove(index);
            if (partitionObserverCache != null) {
              partitionObserverCache.referenceValueRemoved(this, getClassifier().requireReferenceByName("enumeratedValue"), index, removed);
            }
          } else {
            throw new IllegalArgumentException(
                "Invalid index "
                    ...
                    + enumeratedValue.size());
          }
  }

  public void setEnumeratedValue(@NotNull List<? extends ReferenceValue> newValue) {
    clearEnumeratedValue();
          for (ReferenceValue referenceValue : newValue) {
              addToEnumeratedValue(referenceValue, enumeratedValue.size());
          }
  }
  ...
  public List<ReferenceValue> getReferenceValues(@NotNull Reference reference) {
    ...
    if (Objects.equals(reference.getKey(), "sysml-EnumerationDefinition-enumeratedValue")) {
      return enumeratedValue;
    }
  ...
  public int addReferenceValue(Reference reference, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-EnumerationDefinition-enumeratedValue")) {
      return addToEnumeratedValue(referredNode, enumeratedValue.size());
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public int addReferenceValue(Reference reference, int index, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-EnumerationDefinition-enumeratedValue")) {
      return addToEnumeratedValue(referredNode, index);
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public void setReferred(@NotNull Reference reference, int index, @Nullable Node referredNode) {
    ...
    if (index < 0) throw new IllegalArgumentException("index should be non-negative");;
    if (Objects.equals(reference.getKey(), "sysml-EnumerationDefinition-enumeratedValue")) {
      if (index >= enumeratedValue.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = enumeratedValue.get(index);
      enumeratedValue.set(index, original.withReferred(referredNode));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-Definition-variant")) {
  ...
  public void setResolveInfo(@NotNull Reference reference, int index,
    ...
    if (Objects.equals(reference.getKey(), "sysml-EnumerationDefinition-enumeratedValue")) {
      if (index >= enumeratedValue.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = enumeratedValue.get(index);
      enumeratedValue.set(index, original.withResolveInfo(resolveInfo));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-Definition-variant")) {
      if (index >= variant.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
