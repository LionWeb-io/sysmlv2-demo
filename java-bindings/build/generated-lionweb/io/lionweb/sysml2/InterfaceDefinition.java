...
public class InterfaceDefinition extends ConnectionDefinition {
  ...
  protected List<ReferenceValue> interfaceEnd = new ArrayList<>();

  public InterfaceDefinition(@NotNull String id) {
    super(id);
  }
  ...
  public Concept getClassifier() {
    return SysmlLanguage.getInstance().getInterfaceDefinition();
  }

  public int addToInterfaceEnd(ReferenceValue referenceValue, int index) {
    if (index > interfaceEnd.size()) {
      throw new IllegalArgumentException("Index must be less than or equal to size");
    }
    if (partitionObserverCache != null) {
      partitionObserverCache.referenceValueAdded(this, this.getClassifier().requireReferenceByName("interfaceEnd"), index, referenceValue);
    }
    interfaceEnd.add(index, referenceValue);
    return interfaceEnd.size() - 1;
  }

  public List<ReferenceValue> getInterfaceEnd() {
    return interfaceEnd;
  }

  public int addToInterfaceEnd(PortUsage referred) {
    return addToInterfaceEnd(new ReferenceValue(referred, null), interfaceEnd.size());
  }

  public int addToInterfaceEnd(PortUsage referred, int index) {
    return addToInterfaceEnd(new ReferenceValue(referred, null), index);
  }

  public void clearInterfaceEnd() {
    while (!interfaceEnd.isEmpty()) {
            removeFromInterfaceEnd(0);
        };
  }

  public void removeFromInterfaceEnd(@NotNull ReferenceValue child) {
    int index = interfaceEnd.indexOf(child);
             if (index == -1) {
                 throw new IllegalArgumentException("Child not found: " + child);
             }
             removeFromInterfaceEnd(index);;
  }

  public void removeFromInterfaceEnd(int index) {
    if (interfaceEnd.size() > index) {

            ReferenceValue removed = interfaceEnd.remove(index);
            if (partitionObserverCache != null) {
              partitionObserverCache.referenceValueRemoved(this, getClassifier().requireReferenceByName("interfaceEnd"), index, removed);
            }
          } else {
  ...
  public void setInterfaceEnd(@NotNull List<? extends ReferenceValue> newValue) {
    clearInterfaceEnd();
          for (ReferenceValue referenceValue : newValue) {
              addToInterfaceEnd(referenceValue, interfaceEnd.size());
          }
  }
  ...
  public List<ReferenceValue> getReferenceValues(@NotNull Reference reference) {
    ...
    if (Objects.equals(reference.getKey(), "sysml-InterfaceDefinition-interfaceEnd")) {
      return interfaceEnd;
    }
  ...
  public int addReferenceValue(Reference reference, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-InterfaceDefinition-interfaceEnd")) {
      return addToInterfaceEnd(referredNode, interfaceEnd.size());
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public int addReferenceValue(Reference reference, int index, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-InterfaceDefinition-interfaceEnd")) {
      return addToInterfaceEnd(referredNode, index);
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public void setReferred(@NotNull Reference reference, int index, @Nullable Node referredNode) {
    ...
    if (index < 0) throw new IllegalArgumentException("index should be non-negative");;
    if (Objects.equals(reference.getKey(), "sysml-InterfaceDefinition-interfaceEnd")) {
      if (index >= interfaceEnd.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = interfaceEnd.get(index);
      interfaceEnd.set(index, original.withReferred(referredNode));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-ConnectionDefinition-connectionEnd")) {
  ...
  public void setResolveInfo(@NotNull Reference reference, int index,
    ...
    if (Objects.equals(reference.getKey(), "sysml-InterfaceDefinition-interfaceEnd")) {
      if (index >= interfaceEnd.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = interfaceEnd.get(index);
      interfaceEnd.set(index, original.withResolveInfo(resolveInfo));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-ConnectionDefinition-connectionEnd")) {
      if (index >= connectionEnd.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
