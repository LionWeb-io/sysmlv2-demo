...
public class InterfaceUsage extends ConnectionUsage {
  ...
  protected List<ReferenceValue> interfaceDefinition = new ArrayList<>();

  public InterfaceUsage(@NotNull String id) {
    super(id);
  }
  ...
  public Concept getClassifier() {
    return SysmlLanguage.getInstance().getInterfaceUsage();
  }

  public int addToInterfaceDefinition(ReferenceValue referenceValue, int index) {
    if (index > interfaceDefinition.size()) {
      throw new IllegalArgumentException("Index must be less than or equal to size");
    }
    if (partitionObserverCache != null) {
      partitionObserverCache.referenceValueAdded(this, this.getClassifier().requireReferenceByName("interfaceDefinition"), index, referenceValue);
    }
    interfaceDefinition.add(index, referenceValue);
    return interfaceDefinition.size() - 1;
  }

  public List<ReferenceValue> getInterfaceDefinition() {
    return interfaceDefinition;
  }

  public int addToInterfaceDefinition(InterfaceDefinition referred) {
    return addToInterfaceDefinition(new ReferenceValue(referred, null), interfaceDefinition.size());
  }

  public int addToInterfaceDefinition(InterfaceDefinition referred, int index) {
    return addToInterfaceDefinition(new ReferenceValue(referred, null), index);
  }

  public void clearInterfaceDefinition() {
    while (!interfaceDefinition.isEmpty()) {
            removeFromInterfaceDefinition(0);
        };
  }

  public void removeFromInterfaceDefinition(@NotNull ReferenceValue child) {
    int index = interfaceDefinition.indexOf(child);
             if (index == -1) {
                 throw new IllegalArgumentException("Child not found: " + child);
             }
             removeFromInterfaceDefinition(index);;
  }

  public void removeFromInterfaceDefinition(int index) {
    if (interfaceDefinition.size() > index) {

            ReferenceValue removed = interfaceDefinition.remove(index);
            if (partitionObserverCache != null) {
              partitionObserverCache.referenceValueRemoved(this, getClassifier().requireReferenceByName("interfaceDefinition"), index, removed);
            }
          } else {
            throw new IllegalArgumentException(
                "Invalid index "
                    ...
                    + interfaceDefinition.size());
          }
  }

  public void setInterfaceDefinition(@NotNull List<? extends ReferenceValue> newValue) {
    clearInterfaceDefinition();
          for (ReferenceValue referenceValue : newValue) {
              addToInterfaceDefinition(referenceValue, interfaceDefinition.size());
          }
  }
  ...
  public List<ReferenceValue> getReferenceValues(@NotNull Reference reference) {
    ...
    if (Objects.equals(reference.getKey(), "sysml-InterfaceUsage-interfaceDefinition")) {
      return interfaceDefinition;
    }
  ...
  public int addReferenceValue(Reference reference, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-InterfaceUsage-interfaceDefinition")) {
      return addToInterfaceDefinition(referredNode, interfaceDefinition.size());
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public int addReferenceValue(Reference reference, int index, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-InterfaceUsage-interfaceDefinition")) {
      return addToInterfaceDefinition(referredNode, index);
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public void setReferred(@NotNull Reference reference, int index, @Nullable Node referredNode) {
    ...
    if (index < 0) throw new IllegalArgumentException("index should be non-negative");;
    if (Objects.equals(reference.getKey(), "sysml-InterfaceUsage-interfaceDefinition")) {
      if (index >= interfaceDefinition.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = interfaceDefinition.get(index);
      interfaceDefinition.set(index, original.withReferred(referredNode));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-ConnectionUsage-connectionDefinition")) {
  ...
  public void setResolveInfo(@NotNull Reference reference, int index,
    ...
    if (Objects.equals(reference.getKey(), "sysml-InterfaceUsage-interfaceDefinition")) {
      if (index >= interfaceDefinition.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = interfaceDefinition.get(index);
      interfaceDefinition.set(index, original.withResolveInfo(resolveInfo));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-ConnectionUsage-connectionDefinition")) {
      if (index >= connectionDefinition.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
