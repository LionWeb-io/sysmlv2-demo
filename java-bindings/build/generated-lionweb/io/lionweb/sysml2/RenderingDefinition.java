...
public class RenderingDefinition extends PartDefinition {
  @NotNull
  private String id;
  ...
  protected List<ReferenceValue> rendering = new ArrayList<>();

  public RenderingDefinition(@NotNull String id) {
    super(id);
  }
  ...
  public Concept getClassifier() {
    return SysmlLanguage.getInstance().getRenderingDefinition();
  }

  public int addToRendering(ReferenceValue referenceValue, int index) {
    if (index > rendering.size()) {
      throw new IllegalArgumentException("Index must be less than or equal to size");
    }
    if (partitionObserverCache != null) {
      partitionObserverCache.referenceValueAdded(this, this.getClassifier().requireReferenceByName("rendering"), index, referenceValue);
    }
    rendering.add(index, referenceValue);
    return rendering.size() - 1;
  }

  public List<ReferenceValue> getRendering() {
    return rendering;
  }

  public int addToRendering(RenderingUsage referred) {
    return addToRendering(new ReferenceValue(referred, null), rendering.size());
  }

  public int addToRendering(RenderingUsage referred, int index) {
    return addToRendering(new ReferenceValue(referred, null), index);
  }

  public void clearRendering() {
    while (!rendering.isEmpty()) {
            removeFromRendering(0);
        };
  }

  public void removeFromRendering(@NotNull ReferenceValue child) {
    int index = rendering.indexOf(child);
             if (index == -1) {
  ...
  public void removeFromRendering(int index) {
    if (rendering.size() > index) {

            ReferenceValue removed = rendering.remove(index);
            if (partitionObserverCache != null) {
              partitionObserverCache.referenceValueRemoved(this, getClassifier().requireReferenceByName("rendering"), index, removed);
            }
          } else {
  ...
  public void setRendering(@NotNull List<? extends ReferenceValue> newValue) {
    clearRendering();
          for (ReferenceValue referenceValue : newValue) {
              addToRendering(referenceValue, rendering.size());
          }
  }
  ...
  public List<ReferenceValue> getReferenceValues(@NotNull Reference reference) {
    ...
    if (Objects.equals(reference.getKey(), "sysml-RenderingDefinition-rendering")) {
      return rendering;
    }
  ...
  public int addReferenceValue(Reference reference, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-RenderingDefinition-rendering")) {
      return addToRendering(referredNode, rendering.size());
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public int addReferenceValue(Reference reference, int index, ReferenceValue referredNode) {
    if (Objects.equals(reference.getKey(), "sysml-RenderingDefinition-rendering")) {
      return addToRendering(referredNode, index);
    }
    throw new IllegalStateException("Reference " + reference + " not found.");
  }
  ...
  public void setReferred(@NotNull Reference reference, int index, @Nullable Node referredNode) {
    ...
    if (index < 0) throw new IllegalArgumentException("index should be non-negative");;
    if (Objects.equals(reference.getKey(), "sysml-RenderingDefinition-rendering")) {
      if (index >= rendering.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = rendering.get(index);
      rendering.set(index, original.withReferred(referredNode));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-OccurrenceDefinition-lifeClass")) {
  ...
  public void setResolveInfo(@NotNull Reference reference, int index,
    ...
    if (Objects.equals(reference.getKey(), "sysml-RenderingDefinition-rendering")) {
      if (index >= rendering.size()) throw new IllegalArgumentException("index should be less than the size of the list");;
      ReferenceValue original = rendering.get(index);
      rendering.set(index, original.withResolveInfo(resolveInfo));
      return;
    }
    if (Objects.equals(reference.getKey(), "sysml-OccurrenceDefinition-lifeClass")) {
      if (index >= 1 || lifeClass == null) throw new IllegalArgumentException("index should be less than the size of the list");;
